...
**방법 A — 노트북**  
`notebooks/DP08_colab_7B.ipynb` 를 Colab에 업로드하거나, 레포 clone 후 해당 노트북을 연 뒤 **GPU 런타임**으로 위에서 아래로 실행.
...
### 4.2 미니 RAG (임베딩 retrieve)

`data/places.json`(이름·지역·태그·한 줄 설명)에서 질문과 관련된 장소를 **top-k** 로 골라 프롬프트 `[검색 컨텍스트]`에 넣습니다. 검색은 `retrieve()`만 임베딩으로 교체했고, 주입·허용 목록은 그대로입니다.

| 구성 | 경로 |
|------|------|
| 장소 DB | `data/places.json` (~48곳, 태그·설명 보강) |
| 검색 | `app/rag.py` (`retrieve`, MiniLM 코사인 + 키워드 하이브리드) |
| 폴백 | 임베딩 로드 실패 또는 `INDOOR_RAG_BACKEND=keyword` → 키워드 점수 |
| 허용 목록 | `PLACE_BANK` ← `places.json` 자동 동기화 |
| 주입 | `IndoorChatbotModel._build_chat` |
| ON/OFF | UI 체크박스 / `INDOOR_RAG=0` 환경변수 |

임베딩 모델은 **CPU**에 올려 Qwen(3B) MPS 메모리와 겹치지 않게 했습니다. 장소 벡터는 `data/.cache/`에 저장해 재시작 시 재계산을 피합니다.
...
### 5.5 Colab 7B 실내 추천 비교 ✅

원본: [`assets/indoor_compare_7B_colab.md`](./assets/indoor_compare_7B_colab.md)  
환경: Colab T4 CUDA, DP08 SYSTEM_PROMPT + 가드레일 (`compare_indoor_colab.py`)

| 시나리오 | 초 | 재생성 | 목록매칭 | 요약 |
|----------|----|--------|----------|------|
| 기본 실내 데이트 | 37.2 | - | 디뮤지엄, 예술의전당 | 형식 `1) 2)` 준수, 실내 OK |
| 야외유도 한강 | 49.8 | - | DDP, 코엑스몰 | 가드레일 없이도 실내 대안 |
| 다른 거 재추천 | 20.4 | - | 예술의전당, 코엑스몰 | 이전(박물관·디뮤지엄) 제외 성공 |

**체감:** 7B는 형식·실내 제약을 **한 번에** 잘 지킴 (야외 시나리오도 재생성 없음).  
Mac 기본 서빙은 여전히 **3B + 미니 RAG**가 현실적이고, 7B는 Colab 품질 상한 참고용.

## 6. 회고

**배운 점**
- 서빙 파이프라인(FastAPI·인증·Streamlit)은 Day1~7을 조립하면 되고, 제품 차이는 **제약(실내)을 프롬프트·가드레일·목록으로 얼마나 강제하느냐**에 있었다.
- 모델이 크다고 항상 좋은 것은 아님(Day7). 다만 DP08 프롬프트 아래 **7B(Colab)는 형식·실내 제약을 재생성 없이 잘 지켰고**, Mac 서빙 기본은 **3B + 미니 RAG**가 현실적이다.
- 환각은 “더 큰 모델”만으로 안 줄고, **검증 목록 + 미니 RAG(검색 컨텍스트)** 가 체감이 컸다.

**아쉬운 점**
- 임베딩 RAG는 장소 48곳 규모에서는 잘 맞지만, 영업시간·지도 링크 같은 외부 사실은 여전히 검증하지 않음.
- 사이드바「로드됨」은 채팅 응답의 `model_key`와 `/health`로 동기화한다. 선택 모델과 다르면 “다음 요청 시 교체” 안내를 띄운다.

**실행 환경**
- Mac MPS / 기본 **3B** + 미니 RAG, 경량 시 1.5B
- Colab T4 7B 실내 비교: `assets/indoor_compare_7B_colab.md`
...
- [x] `retrieve()` 임베딩 교체 (keyword 폴백 유지)
- [x] Colab 7B 실내 추천 비교 → `assets/indoor_compare_7B_colab.md`
