...
flowchart LR
  ...
  Gen --> Prompt["실내 전용 system prompt + PLACE_BANK"]
...
**한계:** 목록 밖 사실을 검증하는 RAG는 아님. 영업시간·“무료” 같은 단정, `DPP` 오타 등은 남을 수 있음.
...
- 환각은 “더 큰 모델”만으로 안 줄고, **검증 목록을 프롬프트에 넣는 방식**이 체감이 컸다.

**아쉬운 점**
- 키워드/패턴 휴리스틱은 오탐·미탐 가능. 다음 단계는 장소 DB·RAG 또는 공식 API 연동.
