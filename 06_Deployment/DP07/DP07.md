...
7B Colab 원본: [`assets/model_compare_7B_colab.md`](./assets/model_compare_7B_colab.md)  
재실행(Mac): `.venv/bin/python scripts/compare_models.py --models 0.5B 1.5B 3B`  
재실행(Colab): `!python scripts/compare_models.py --models 7B`
...
| 크기 | 로드 | 인사 | 멀티턴 | 지시준수 |
|------|------|------|--------|----------|
| 7B | 65.9s | 9.4s — OK | 45.4s — 미술관(실내) OK, 두 번째 추천은 환각/모호 | 1.3s — `Apple` |

**결론:** 실습 파이프라인은 **1.5B면 충분**. 같은 프롬프트 기준 **체감 품질 점프는 3B가 가장 큼**. 7B는 Mac MPS에서 생성 불가에 가깝고, Colab CUDA에서는 정상 완료되나 **이 테스트만 보면 3B 대비 이득은 제한적**.

| 단계 | 모델 | 상태 |
|------|------|------|
| 1 | 0.5B-Instruct | ✅ Mac MPS |
| 2 | 1.5B-Instruct (기본 실습) | ✅ Mac MPS |
| 3 | 3B-Instruct | ✅ Mac MPS |
| 4 | 7B-Instruct | ✅ Colab T4 (`model_compare_7B_colab.md`) |
...
- 1.5B는 한국어·지시 준수가 가끔 약하다 → **0.5B/1.5B/3B/7B 비교**: **3B에서 실내 지시 준수가 크게 개선**. 7B(Colab)는 동작은 안정이나 이 프롬프트에선 3B 대비 체감 이득이 크지 않음.
- UI/통합 테스트 스크린샷은 `assets/ui_test_*.png`에 정리 완료.

**실행 환경 메모**

- 커널: `DP07 .venv`
- 기본 모델: `Qwen/Qwen2.5-1.5B-Instruct` (mps)
- 7B 비교: Colab T4 + `scripts/compare_models.py --models 7B`
